locals {
  ...
  func_name = format(var.vm_provision_adapter_func_name_template, local.region_env)
  func_app_hostname = try(data.azurerm_windows_function_app.vm_provision_adapter_windows_function_app.default_hostname, data.azurerm_linux_function_app.vm_provision_adapter_linux_function_app.default_hostname)
  func_app_id = try(data.azurerm_windows_function_app.vm_provision_adapter_windows_function_app.id, data.azurerm_linux_function_app.vm_provision_adapter_linux_function_app.id)
  func_openapi_document_url = format("%s%s/api/openapi/v3.json", "https://", local.func_app_hostname)

  vm-set-backend-service-policy = <<XML
  <set-backend-service base-url="https://${local.func_app_hostname}/api" />
  XML

  API_BACKEND_URL_PLACEHOLDER = "<!--API BACKEND URL PLACEHOLDER-->"
  vm-provision-api-policy-xml-content = replace(local.apim-api-policy-xml-content_template, local.API_BACKEND_URL_PLACEHOLDER, local.vm-set-backend-service-policy)
