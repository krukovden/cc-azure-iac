...
variable vm_provision_adapter_func_name_template {
  default = "func-cc-cp-vm-provision-adapter-%s"
}

variable "vm_provision_adapter_apim_api_params" {
  default = {
    name        = "abs-cc-vm-provision-adapters-api"
    path        = "vmprovisionadapters"
    displayName = "Voyage Manager Adapter Management API"
    revision    = "1"
    description = "Voyage Manager Adapter Management API"
    protocols   = ["https"]
  }
}
