locals {
  ...
  vesonports_func_name = format(var.vesonports_func_name_template, local.region_env)
  vesonports_func_app_hostname = try(data.azurerm_windows_function_app.vesonports_windows_function_app.default_hostname, data.azurerm_linux_function_app.vesonports_linux_function_app.default_hostname)
  vesonports_func_app_id = try(data.azurerm_windows_function_app.vesonports_windows_function_app.id, data.azurerm_linux_function_app.vesonports_linux_function_app.id)
  vesonports_func_openapi_document_url = format("%s%s/api/openapi/v3.json", "https://", local.vesonports_func_app_hostname)

  vesonports-set-backend-service-policy = <<XML
  <set-backend-service base-url="https://${local.vesonports_func_app_hostname}/api" />
  XML

  API_BACKEND_URL_PLACEHOLDER = "<!--API BACKEND URL PLACEHOLDER-->"
  vesonports-api-policy-xml-content = replace(local.apim-api-policy-xml-content_template, local.API_BACKEND_URL_PLACEHOLDER, local.vesonports-set-backend-service-policy)
