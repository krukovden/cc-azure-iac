...
variable onboardingmiddleware_func_name_template {
  default = "func-cc-cp-onboarding-middleware-api-%s"
}

variable "onboardingmiddleware_apim_api_params" {
  default = {
    name        = "abs-cc-onboarding-middleware-api"
    path        = "onboardingmiddleware"
    displayName = "OnboardingMiddleware API"
    revision    = "1"
    description = "OnboardingMiddleware API"
    protocols   = ["https"]
  }
}
