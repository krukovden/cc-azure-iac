locals {
  ...
  onboardingmiddleware_func_name = format(var.onboardingmiddleware_func_name_template, local.region_env)
  onboardingmiddleware_func_app_hostname = try(data.azurerm_windows_function_app.onboardingmiddleware_windows_function_app.default_hostname, data.azurerm_linux_function_app.onboardingmiddleware_linux_function_app.default_hostname)
  onboardingmiddleware_func_app_id = try(data.azurerm_windows_function_app.onboardingmiddleware_windows_function_app.id, data.azurerm_linux_function_app.onboardingmiddleware_linux_function_app.id)
  onboardingmiddleware_func_openapi_document_url = format("%s%s/api/openapi/v3.json", "https://", local.onboardingmiddleware_func_app_hostname)

  onboardingmiddleware-set-backend-service-policy = <<XML
  <set-backend-service base-url="https://${local.onboardingmiddleware_func_app_hostname}/api" />
  XML

  API_BACKEND_URL_PLACEHOLDER = "<!--API BACKEND URL PLACEHOLDER-->"
  onboardingmiddleware-api-policy-xml-content = replace(local.apim-api-policy-xml-content_template, local.API_BACKEND_URL_PLACEHOLDER, local.onboardingmiddleware-set-backend-service-policy)
