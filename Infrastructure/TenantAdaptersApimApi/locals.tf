locals {
  ...
  tenants_func_name = format(var.tenants_func_name_template, local.region_env)
  tenants_func_app_hostname = try(data.azurerm_windows_function_app.tenants_windows_function_app.default_hostname, data.azurerm_linux_function_app.tenants_linux_function_app.default_hostname)
  tenants_func_app_id = try(data.azurerm_windows_function_app.tenants_windows_function_app.id, data.azurerm_linux_function_app.tenants_linux_function_app.id)
  tenants_func_openapi_document_url = format("%s%s/api/openapi/v3.json", "https://", local.tenants_func_app_hostname)

  tenants-set-backend-service-policy = <<XML
  <set-backend-service base-url="https://${local.tenants_func_app_hostname}/api" />
  XML

  API_BACKEND_URL_PLACEHOLDER = "<!--API BACKEND URL PLACEHOLDER-->"
  tenants-api-policy-xml-content = replace(local.apim-api-policy-xml-content_template, local.API_BACKEND_URL_PLACEHOLDER, local.tenants-set-backend-service-policy)
