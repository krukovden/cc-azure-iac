locals {
  ...
  waypoints_func_name = format(var.waypoints_func_name_template, local.region_env)
  waypoints_func_app_hostname = try(data.azurerm_windows_function_app.waypoints_windows_function_app.default_hostname, data.azurerm_linux_function_app.waypoints_linux_function_app.default_hostname)
  waypoints_func_app_id = try(data.azurerm_windows_function_app.waypoints_windows_function_app.id, data.azurerm_linux_function_app.waypoints_linux_function_app.id)
  waypoints_func_openapi_document_url = format("%s%s/api/openapi/v3.json", "https://", local.waypoints_func_app_hostname)

  waypoints-set-backend-service-policy = <<XML
  <set-backend-service base-url="https://${local.waypoints_func_app_hostname}/api" />
  XML

  API_BACKEND_URL_PLACEHOLDER = "<!--API BACKEND URL PLACEHOLDER-->"
  waypoints-api-policy-xml-content = replace(local.apim-api-policy-xml-content_template, local.API_BACKEND_URL_PLACEHOLDER, local.waypoints-set-backend-service-policy)
