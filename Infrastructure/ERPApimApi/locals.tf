locals {
  ...
  erp_getreportsof_func_name = format(var.erp_getreportsof_func_name_template, local.region_env)
  erp_getreportsof_func_app_hostname = try(data.azurerm_windows_function_app.erp_getreportsof_windows_function_app.default_hostname, data.azurerm_linux_function_app.erp_getreportsof_linux_function_app.default_hostname)
  erp_getreportsof_func_app_id = try(data.azurerm_windows_function_app.erp_getreportsof_windows_function_app.id, data.azurerm_linux_function_app.erp_getreportsof_linux_function_app.id)
  erp_getreportsof_func_openapi_document_url = format("%s%s/api/openapi/v3.json", "https://", local.erp_getreportsof_func_app_hostname)

  erp-getreportsof-set-backend-service-policy = <<XML
  <set-backend-service base-url="https://${local.erp_getreportsof_func_app_hostname}/api" />
  XML

  API_BACKEND_URL_PLACEHOLDER = "<!--API BACKEND URL PLACEHOLDER-->"
 erp-getreportsof-api-policy-xml-content = replace(local.apim-api-policy-xml-content_template, local.API_BACKEND_URL_PLACEHOLDER, local.erp-getreportsof-set-backend-service-policy)
