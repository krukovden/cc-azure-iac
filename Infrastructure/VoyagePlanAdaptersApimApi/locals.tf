locals {
  ...
  voyageplan_func_name = format(var.voyageplan_func_name_template, local.region_env)
  voyageplan_func_app_hostname = try(data.azurerm_windows_function_app.voyageplan_windows_function_app.default_hostname, data.azurerm_linux_function_app.voyageplan_linux_function_app.default_hostname)
  voyageplan_func_app_id = try(data.azurerm_windows_function_app.voyageplan_windows_function_app.id, data.azurerm_linux_function_app.voyageplan_linux_function_app.id)
  voyageplan_func_openapi_document_url = format("%s%s/api/openapi/v3.json", "https://", local.voyageplan_func_app_hostname)

  voyageplan-set-backend-service-policy = <<XML
  <set-backend-service base-url="https://${local.voyageplan_func_app_hostname}/api" />
  XML

  API_BACKEND_URL_PLACEHOLDER = "<!--API BACKEND URL PLACEHOLDER-->"
  voyageplan-api-policy-xml-content = replace(local.apim-api-policy-xml-content_template, local.API_BACKEND_URL_PLACEHOLDER, local.voyageplan-set-backend-service-policy)
